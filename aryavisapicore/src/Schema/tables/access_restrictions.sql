...
CREATE TABLE `access_restrictions` (
   ...
   `entity_guid` binary(16) NOT NULL,
   `allowed` bit(1) NOT NULL,
   PRIMARY KEY (`id`),
   UNIQUE KEY `UQ_feature_entity` (`feature`,`entity_type`,`entity_guid`) USING BTREE,
   KEY `IDX_feature` (`feature`) USING BTREE,
   KEY `IDX_entity` (`entity_type`,`entity_guid`) USING BTREE
